...
public class ReportController {
        ...
        public ApiResponse<Page<ReportResponse>> getMyReports(
                ...
                Sort.Direction sortDirection = "asc".equalsIgnoreCase(direction) ? Sort.Direction.ASC
                                : Sort.Direction.DESC;
        ...
        @DeleteMapping("/{reportRefId}")
        @PreAuthorize("hasRole('ORG_ADMIN')")
        public ApiResponse<DeleteReportResponse> deleteReport(
                        @PathVariable String reportRefId) {
                return ApiResponse.success(
                                "Report deleted successfully",
                                reportService.deleteReport(reportRefId));
        }

        // 10. BULK DELETE REPORTS (SELECTED)

        @DeleteMapping("/bulk")
        @PreAuthorize("hasRole('ORG_ADMIN')")
        public ApiResponse<BulkDeleteReportsResponse> deleteReports(
                        @Valid @RequestBody BulkDeleteReportsRequest request) {
                return ApiResponse.success(
                                "Reports processed for deletion",
                                reportService.deleteReports(request));
        }

        // 11. BULK DELETE REPORTS BY DATE RANGE

        @DeleteMapping("/bulk/date-range")
        @PreAuthorize("hasRole('ORG_ADMIN')")
        public ApiResponse<BulkDeleteReportsResponse> deleteReportsByDateRange(
                        @Valid @RequestBody DeleteReportsByDateRangeRequest request) {
                return ApiResponse.success(
