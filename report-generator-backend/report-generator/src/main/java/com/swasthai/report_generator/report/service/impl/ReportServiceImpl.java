...
public class ReportServiceImpl implements ReportService {
        ...
        public ReportResponse createReport(
                ...
                /*
                 * Authentication is resolved from the server-side
                 * SecurityContext.
                 *
                 * We NEVER take organization identity from the request.
                 */
                 ...
                 * ============================================================
                 *
                 * This MUST execute before report creation.
                 *
                 * The organization ID comes from the authenticated user's
                 * server-side organization relationship.
                 *
                 * The client cannot select another organization here.
                 */
                licenseGuard.requireReportCreationAllowed(
                                organization.getId());

                /*
                 * Verify patient belongs to this same organization.
                 */
                ...
                /*
                 * Only server-controlled values are used.
                 *
                 * No status, organization, createdAt, license state,
                 * or creator information comes from the client.
                 */
        ...
        public ReportResponse addTest(
                ...
                for (TestParameter param : activeParams) {
                        ...
                        TestParameterResult paramResult = TestParameterResult.builder()
                                        ...
                                        .calculationVersion(
                                                        param.getInputType() == ParameterInputType.CALCULATED
        ...
        public ReportResponse removeTest(
                ...
                ReportTestResult targetTest = report.getTests()
                                .stream()
                                .filter(t -> t.getRefId()
        ...
        public ReportResponse updateParameterValues(
                ...
                ReportTestResult reportTest = report.getTests()
                                .stream()
                                .filter(t -> t.getRefId()
                                                .equals(reportTestRefId.trim()))
                                .findFirst()
                ...
                for (TestParameterResultInput input : request.parameters()) {
                        ...
                        TestParameterResult matched = resolveParameterResult(
                                        input,
                                        resultsByRefId,
                                        resultsByCode);
                        ...
                        // Manual parameter
                        String rawVal = input.value() != null
                                        ? input.value().trim()
                                        : null;
                        ...
                        if (rawVal != null
                                ...
                                BigDecimal parsedNumber = parseStrictNumeric(
                                                code,
                                                rawVal);
        ...
        public ReportResponse reorderTests(
                ...
                Map<String, ReportTestResult> testsByRefId = report.getTests()
                                .stream()
                                .collect(
                                                Collectors.toMap(
                ...
                for (TestOrderItemInput orderInput : request.testOrders()) {
                        ...
                        ReportTestResult test = testsByRefId.get(
                ...
                for (ReportTestResult reportTest : report.getTests()) {
                        ...
                        for (TestParameterResult paramResult : reportTest.getParameterResults()) {
                                ...
                                if (paramResult.getInputType() == ParameterInputType.MANUAL) {
                                        ...
                                        boolean isRequired = originalParam == null
        ...
        private void executeCalculationsForTest(
                ...
                for (TestParameterResult result : reportTest.getParameterResults()) {
                        ...
                        if (result.getInputType() == ParameterInputType.MANUAL
                ...
                for (TestParameterResult result : reportTest.getParameterResults()) {
                        ...
                        if (result.getInputType() == ParameterInputType.CALCULATED) {
                                ...
                                Set<String> requiredDeps = calculationEngine
                                ...
                                if (allDepsPresent) {
                                        ...
                                        BigDecimal calculated = calculationEngine.calculate(
                                                        calcType,
                                                        numericValues);
        ...
        private TestParameterResult resolveParameterResult(
                ...
                if (input.parameterRefId() != null
                        ...
                        TestParameterResult res = byRefId.get(
                ...
                if (input.parameterCode() != null
                        ...
                        TestParameterResult res = byCode.get(
                                        input.parameterCode()
                                                        .trim()
                                                        .toUpperCase());
        ...
        private Report findAuthorizedReport(
                ...
                Organization organization = getRequiredOrganization(
                                currentUser);
        ...
        public ReportResponse breakGlassAccess(
                ...
                if (request == null) {
                        throw new IllegalArgumentException("Break-glass access request is required");
                }
                ...
                if (currentUser.getRole() != Role.SUPER_ADMIN) {
                        throw new AccessDeniedException("Only SUPER_ADMIN can perform break-glass emergency report access");
                }

                String targetOrgRefId = request.organizationRefId() != null ? request.organizationRefId().trim() : null;
                String normalizedReportRefId = reportRefId.trim();

                if (targetOrgRefId == null || targetOrgRefId.isBlank()) {
                        throw new IllegalArgumentException("Target organization reference ID is required");
                }

                Organization organization = organizationRepository.findByRefId(targetOrgRefId).orElse(null);
                ...
                if (organization == null) {
                        auditLogService.recordBreakGlassAccess(
                                        ...
                                        clientIp);
                        throw new ResourceNotFoundException("Target organization not found: " + targetOrgRefId);
                }

                Report report = reportRepository.findByRefIdAndOrganization_IdAndDeletedAtIsNull(
                                normalizedReportRefId,
                                organization.getId()).orElse(null);
                ...
                if (report == null) {
                        auditLogService.recordBreakGlassAccess(
                                        ...
                                        clientIp);
                        throw new ResourceNotFoundException("Report not found: " + normalizedReportRefId);
                }
        ...
        private ReportResponse mapToResponse(
                ...
                List<ReportTestItemResponse> testItems = report.getTests()
                                .stream()
                                .sorted(
                                                Comparator.comparingInt(
                                                                ReportTestResult::getDisplayOrder))
        ...
        private ReportTestItemResponse mapTestItem(
                ...
                List<ReportParameterItemResponse> params = reportTest.getParameterResults()
                                .stream()
                                .sorted(
                                                Comparator.comparingInt(
        ...
        public DeleteReportResponse deleteReport(
                ...
                String normalizedRefId = normalizeReportRefId(
                                reportRefId);
        ...
        public BulkDeleteReportsResponse deleteReports(
                ...
                for (int i = 0; i < idList.size(); i += batchSize) {
                        ...
                        int deletedInBatch = deletionBatchExecutor
                ...
                int skippedCount = normalizedRefIds.size()
                                - totalDeleted;
        ...
        public BulkDeleteReportsResponse deleteReportsByDateRange(
                 ...
                 * 2. retention cutoff
                 */
                Instant effectiveEnd = endExclusive.isBefore(
                                retentionCutoff)
                                                ? endExclusive
                                                : retentionCutoff;
                ...
                if (!start.isBefore(effectiveEnd)) {
                        ...
                        long skippedYoung = reportRepository
        ...
        public int purgeExpiredReports() {
                ...
                if (Boolean.FALSE.equals(retentionProperties.getPurgeEnabled())) {
                ...
                Instant purgeCutoff = now.minus( retentionProperties.getPurgeAfterDays(), ChronoUnit.DAYS);
        ...
        private int toSafeInt(long value) {
                ...
                if (value > Integer.MAX_VALUE) {
                        ...
                        throw new IllegalStateException( "Deletion count exceeds supported response range");
                }
