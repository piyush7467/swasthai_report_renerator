...
@Table(
        ...
        uniqueConstraints = {
                @UniqueConstraint(
                        name = "uk_report_test_results_ref_id",
                        columnNames = "ref_id"
                ),
                @UniqueConstraint(
                        name = "uk_report_test_results_report_test",
                        columnNames = {"report_id", "test_id"}
                )
        },
        indexes = {
                @Index(
                        name = "idx_report_test_results_report",
                        columnList = "report_id"
                ),
                @Index(
                        name = "idx_report_test_results_test",
                        columnList = "test_id"
                ),
                @Index(
                        name = "idx_report_test_results_ref_id",
                        columnList = "ref_id"
                ),
                @Index(
                        name = "idx_report_test_results_order",
                        columnList = "report_id,display_order"
                )
        }
)
@Getter
...
public class ReportTestResult {
    ...
    @JoinColumn(
            ...
            foreignKey = @ForeignKey(
                    name = "fk_report_test_results_report"
            )
    )
    ...
    @JoinColumn(
            ...
            nullable = false,
            foreignKey = @ForeignKey(
                    name = "fk_report_test_results_test"
            )
    )
    private Test test;
